    ...
    public void CreateBrickArray() {
        ...
        for (int y = 0; y < brickCountY; y++) {
            for (int x = -(brickCountX / 2); x < (brickCountX / 2); x++) {
                Vector3 pos = new Vector3(0.8f + (x * 1.6f), 1 + (y * 0.4f),
                    0); //The 'pos' variable is where the brick will spawn at
                GameObject brick =
                    Instantiate(brickPrefab, pos,
                        Quaternion.identity) as GameObject; //Creates a new brick game object at the 'pos' value
